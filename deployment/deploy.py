...
flags.DEFINE_string("agent_name", "google-agent-cvm", "display name of the agent")
flags.DEFINE_string("agent_sa", None, "service account of the agent in reasoning engine")
flags.DEFINE_string("wheel_file", "my_agent-0.1-py3-none-any.whl", "agent python wheel file")
flags.DEFINE_string("config_file", "config.json", "the path to the deployment config file")
flags.DEFINE_bool("create", False, "Create a new agent.")
flags.DEFINE_bool("delete", False, "Delete an existing agent.")
flags.DEFINE_bool("update", False, "Update and existing agent")
flags.mark_bool_flags_as_mutual_exclusive(["create", "delete", "update"])

# AGENT_WHL_FILE = "my_agent-0.1-py3-none-any.whl"
...
def create(env_vars: dict[str, str]) -> None:
    ...
    AGENT_WHL_FILE = FLAGS.wheel_file
    adk_app = AdkApp(
        agent=root_agent,
    ...
    remote_agent = agent_engines.create(
        adk_app,
        display_name=FLAGS.agent_name,
        requirements=[AGENT_WHL_FILE],
        extra_packages=[AGENT_WHL_FILE],
        env_vars=env_vars,
        service_account=FLAGS.agent_sa
    )
    ...
    if FLAGS.config_file:
        if os.path.exists(FLAGS.config_file):
            ...
            config_data['reasoning_enigne_id'] = remote_agent.resource_name.split('/')[-1]
            with open(FLAGS.config_file, 'w') as f:
                json.dump(config_data, f, indent=4)
            logger.info(f"Write reasoning_enigne_id to config file: {FLAGS.config_file}")

def update(env_vars: dict[str, str], resource_id: str) -> None:
    logger.info("Attempting to update agent: %s", resource_id)
    AGENT_WHL_FILE = FLAGS.wheel_file
    ...
    try:
        ...
        logger.error(
            "An error occurred while updating agent %s: %s", resource_id, e
        )
        print(f"\nError updating agent {resource_id}: {e}")   
...
def main(argv: list[str]) -> None:  # pylint: disable=unused-argument
    ...
    if not bucket_name:
        ...
        return
    if not FLAGS.create and not FLAGS.delete and not FLAGS.update:
        print("\nError: You must specify either --create, --update or --delete flag.")
        return
    if FLAGS.delete and not FLAGS.resource_id:
        ...
        return
    if FLAGS.update and not FLAGS.resource_id:
        print(
            "\nError: --resource_id is required when using the --update flag."
        )
        return
    ...
    try:
        ...
        if FLAGS.create or FLAGS.update:
